...
APP_TITLE = "Minecraft (Python Edition)"
query_result = []
iterator_index = 0
empty = "NULL"

try:
    conn = sql.connect("exams.db")
    c = conn.cursor()
except Error as e:
...
def create_database():
    ...
    create_record = f"INSERT INTO Exams VALUES(1000, {empty}, {empty}, {empty}, {empty}, {empty});"
...
def get_record(question):
    retrieve_record = f"SELECT * FROM Exams WHERE question LIKE \"%{question}%\";"
    record = c.execute(retrieve_record).fetchall()
    return record
...
def search_database(event):
    ...
    if question_query == "" or question_query == "Please enter question here":
        ...
        query_result = get_record(question_query)
...
add_entry_button = Button(bottom_frame,
                    text="Add New Entry", 
                    width=25,  
                    command=show_add_new_entry).grid(
                        row=1,
                        column=0,
                        padx=5,
                        pady=5
